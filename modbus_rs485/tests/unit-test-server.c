...
int main(int argc, char*argv[])
{
    ...
    if (argc > 2) {
      opt = strtol(argv[2], NULL, 8);
    }
    switch (opt){
      case 0:
        printf("read...\n");
        modbus_read_registers(ctx, 0, 8, tab_reg);
        for (i=0; i<8; i++){
          printf("[%d]=0x%x\n",i,tab_reg[i]);
        }
        break;
      case 1:
        printf("write...\n");
        modbus_write_register(ctx, 3, 0x3333);
        modbus_write_register(ctx, 7, 0x7777);
        break;
      default:
        break;
    }
